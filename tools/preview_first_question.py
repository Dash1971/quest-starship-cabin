...
def views():
 ...
 for name,eye,target in SEATS:
  eye=np.array(eye,float);forward=unit(np.asarray(target)-eye)
  right=unit(np.cross([0,1,0],forward));up=unit(np.cross(forward,right))
  result.append((name+'-upward',eye,eye+(forward+up)*5))
 for name,eye in [('standing-upper',[0,1.7,-.8]),('standing-left',[-1.25,1.65,-1.1]),('standing-right',[1.4,1.65,-.8])]:
  result.append((name,eye,[.2,2.15,-2.1]))
 return result
...
def positions(model,seconds):
 ...
 radius=np.sqrt(model['nearRadius']**2+((model['farRadius']*scales)**2-model['nearRadius']**2)*noise(seed))
 angle=noise(seed+np.uint32(1))*2*np.pi
 p[:,1]=np.where(cycles==0,p[:,1],np.sin(angle)*radius)
 p[:,2]=np.where(cycles==0,p[:,2],np.cos(angle)*radius)
...
def project(p,eye,target,w,h):
 ...
 xy=np.stack([w*.5+(delta@right)*scale/z,h*.5-(delta@up)*scale/z],axis=-1)
 return np.where((z>0)[:,None],xy,np.nan)
...
def render(model,seconds,seat,output):
 ...
 for i,(x,y) in enumerate(xy):
  ...
  star=model['stars'][i];sigma=np.sqrt(star['Sigma']**2+.18)
  t=np.clip((star['Flux']-1.6)/1.9,0,1);halo=t*t*(3-2*t)*.035
  extent=sigma*(7 if halo>=.001 else 3.5)
  ...
  core+=np.exp(-.5*((gx+.5-x)**2+(gy+.5-y)**2)/(sigma*sigma*6.25))*halo
