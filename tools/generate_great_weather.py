...
def generate_rows(first, last):
    ...
    for cx, cy, rx, ry, turns in ((.08, .60, .105, .045, 4.6),
        ...
        x += (dx * np.cos(angle) - dy * np.sin(angle) - dx) * rx * 2*np.pi
        y += displaced_y
...
def generate():
    # Bound working memory: 4K source detail does not need multi-GB arrays.
    return np.concatenate([generate_rows(first, min(first+128, HEIGHT))
                           for first in range(0, HEIGHT, 128)])
...
def save_png(path, pixels):
    ...
    data = b'\x89PNG\r\n\x1a\n'
    data += chunk(b'IHDR', struct.pack('>IIBBBBB', pixels.shape[1], pixels.shape[0], 8, 6, 0, 0, 0))
