...
- **M6.2 Release Candidate — implemented and installed, release exit open:** the formation completes a broad readable flight arc in roughly 100 seconds in Living and 120 seconds in Quiet, with a clear position/scale change inside ten seconds, independent station keeping, animated drives, and a coordinated Living course change. Holding `B` previews the current vista's event from its readable middle phase while normal sessions retain the 12–15-minute release timing. The timer now counts only uninterrupted Living time, and returning to Quiet cancels and resets event state. Five destination-specific spatial beds and event cues, progressive dawn/weather response, and slower blackouts remain unified. Thirty-one deterministic captures pass; the expanded harbour audit covers 1,200 seconds, and GLES3/Vulkan ARM64/OpenXR build and Quest installation pass. The app now requests 72 Hz and logs display refresh, frame/CPU/GPU timing, memory, battery temperature, and Android thermal status by vista and mode. Live frame evidence, headset art/audio sign-off, comfort sessions, and the 45-minute thermal run remain.
...
Status: M6.2 Release Candidate built and installed. Sound, light, persistence, release-length event timing, and transitions remain unified; holding `B` exposes every event immediately for review without changing normal playback timing, formation travel is visibly legible inside ten seconds, and Living timers/events reset cleanly on mode changes. The runtime explicitly requests 72 Hz and reports display refresh, late frames, CPU/GPU timing, memory, battery temperature, and Android thermal state by vista and mode. Thirty-one automated captures, extended traffic validation, Android packaging, and Quest installation pass. Live 72 Hz evidence, headset art/audio sign-off, the relevant 20-minute comfort sessions, and the 45-minute thermal run remain before release.
