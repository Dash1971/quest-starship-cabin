...
A new 3,600 km-diameter moon follows a bounded arc at 76,000 km from the giant's centre. Its actual transform supplies one occulting sphere to the planet, ring and atmosphere shaders. These calculate the same sun-ray intersection and approximate finite-source penumbra. The shadow can fall on the rings before it reaches the globe. It is not a texture decal animated independently of the moon.
...
- Moon and shadow clear a model of the actual sloped glazing and four pane openings at the hold-B phase from all four reference seats, with a 6 cm frame margin.
- Relief matches deterministic generation, with valid slope/coverage/height ranges, longitude continuity and linear import.

Unity scene assertions additionally check sphere/mesh radius agreement, uniform/transform synchronization for all three receivers, preview phase, seek order, Quiet reset and relief import. These assertions passed in Unity 6000.5.2f1 during merge review.
...
1. Check out this PR branch from `main`, using Unity **6000.5.2f1**. Run **Starship Cabin → Quiet Watch → Regenerate, Bake and Build Review APK**. The inherited chess probes require a fresh bake. Version: `2.0.0-m7-weather-eclipse` / `20013`, existing Quiet Watch package and review APK path.
...
Merge review completed Unity API/shader compilation, scene assertions, a fresh two-lightmap/18-probe bake, 42 distinct real captures, the ARM64 APK build and package/provenance audit. Review also caught window-frame occlusion in the original preview and corrected the moon arc; all four fixed-seat preview captures now show both the moon and its shared shadow. In-headset visual/stereo, comfort, sustained 72 Hz and thermal acceptance remain open. M7 is not artistically finished until that headset comparison supports the result. Later harbour/Blue Morning/event-variety work remains on the roadmap.
