Shader "StarshipCabin/QuietWatchCruiseStars"
{
    Properties
    {
        ...
        _WrapWidth ("Sector width", Float) = 196608
        _DepthRange ("Proxy depth range", Vector) = (8000,44000,0,0)
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            CBUFFER_START(UnityPerMaterial)
                ...
                float4 _DepthRange;
            CBUFFER_END
            struct A { float4 p:POSITION; float2 uv:TEXCOORD0; float2 data:TEXCOORD1;
            ...
            struct V { float4 p:SV_POSITION; float2 pixel:TEXCOORD0; float sigma:TEXCOORD1;
                half3 energy:TEXCOORD2; UNITY_VERTEX_OUTPUT_STEREO };
            uint mixBits(uint v)
            {
            ...
            V vert(A a)
            {
                ...
                float localCycle=floor((a.p.x+_Travel+_WrapWidth*.5)/_WrapWidth);
                float3 stellarPosition=a.p.xyz;
                stellarPosition.x=a.p.x+_Travel-localCycle*_WrapWidth;
                uint cycle=(uint)(_Sector+localCycle);
                if(cycle!=0u)
                {
                    uint seed=(uint)(a.data.y+1)^cycle*0x9e3779b9u;
                    float depth=lerp(_DepthRange.x,_DepthRange.y,unitHash(seed));
                    stellarPosition.y=lerp(-.72,.72,unitHash(seed+1u))*depth; stellarPosition.z=-depth;
                }
                float fade=1-smoothstep(_WrapWidth*.40,_WrapWidth*.5,abs(stellarPosition.x));
                ...
                if(_ReviewStar>=0) v.energy=abs(a.data.y-_ReviewStar)<.1 ? half3(1,1,1) : half3(0,0,0);
                return v;
            }
            half4 frag(V v):SV_Target
            {
                ...
                return half4(1-exp(-v.energy*core),0);
            }
            ENDHLSL
        }
    }
