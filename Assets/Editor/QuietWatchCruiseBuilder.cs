...
namespace StarshipCabin.EditorTools
{
    internal static class QuietWatchCruiseBuilder
    {
        public static Renderer Build(Transform parent, out Renderer comet)
        {
            ...
            var data = new Vector2[vertices.Length]; var colors = new Color[vertices.Length];
            ...
            for (var i=0; i<catalogue.Length; i++)
            {
                ...
                for (var k=0; k<4; k++)
                {
                    ...
                    data[i*4+k] = new Vector2(star.Sigma,i);
                    colors[i*4+k] = new Color(star.R,star.G,star.B,star.Flux);
                }
                for(var k=0;k<6;k++) indices[i*6+k]=i*4+order[k];
            }
            var mesh = new Mesh { name="First Question Unified Stellar Field", vertices=vertices, uv=uv,
                uv2=data, colors=colors, triangles=indices };
